...
const MOCK_CATEGORIES: CategorieAttributes[] = [
  { name: "CS BASICS" }, { name: "WEB DEV" },   { name: "CS CORE" },
  { name: "DATABASES" }, { name: "NETWORKING" }, { name: "PROGRAMMING" },
  { name: "DESIGN" },    { name: "MOBILE" },     { name: "AI" },
  { name: "DEVOPS" },    { name: "SECURITY" },   { name: "OTHER" },
];

// ─── Mock existing course for edit mode — null = add mode ────────────────────
const MOCK_COURSE: (CourseAttributes & { id: string }) | null = null;
// To test edit mode, set this to a real course object

const MOCK_LESSONS: LessonAttributes[] = [];
...
// ─── Lesson card (expanded / collapsed) ──────────────────────────────────────
interface LessonCardProps {
  lesson: LessonAttributes;
  index: number;
  total: number;
  onChange: (id: string, field: keyof LessonAttributes, value: string) => void;
...
const LessonCard = ({ lesson, index, total, onChange, onDelete, onMoveUp, onMoveDown, dragHandleProps }: LessonCardProps) => {
  ...
  return (
    <div className="bg-[#f5f5fb] rounded-2xl border border-gray-100 overflow-hidden">
      ...
      <div className="flex items-center gap-3 px-4 py-3">
        {/* Drag handle */}
        <div {...dragHandleProps} className="cursor-grab text-gray-300 hover:text-gray-500 shrink-0 select-none" title="Drag to reorder">
        ...
        <div className="flex items-center gap-1 shrink-0">
          <button onClick={() => onMoveUp(index)}   disabled={index === 0}           title="Move up"   className="p-1.5 rounded-lg text-gray-400 hover:text-purple-600 hover:bg-purple-50 disabled:opacity-30 disabled:cursor-not-allowed transition-colors">
          ...
          <button onClick={() => onMoveDown(index)} disabled={index === total - 1}   title="Move down" className="p-1.5 rounded-lg text-gray-400 hover:text-purple-600 hover:bg-purple-50 disabled:opacity-30 disabled:cursor-not-allowed transition-colors">
          ...
          <button onClick={() => setExpanded(!expanded)} title={expanded ? "Collapse" : "Expand"} className="p-1.5 rounded-lg text-gray-400 hover:text-purple-600 hover:bg-purple-50 transition-colors">
          ...
          <button onClick={() => onDelete(lesson.id)} title="Delete lesson" className="p-1.5 rounded-lg text-gray-400 hover:text-red-500 hover:bg-red-50 transition-colors">
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                value={lesson.vedio_url}
                onChange={e => onChange(lesson.id, "vedio_url", e.target.value)}
                placeholder="https://youtube.com/watch?v=... or direct video URL"
              />
            </div>
            {/* Video preview */}
            {lesson.vedio_url && (
              <div className="mt-2 bg-black rounded-xl overflow-hidden aspect-video">
                <iframe
                  src={lesson.vedio_url.replace("watch?v=", "embed/")}
...
const AddCourse = () => {
  ...
  const isEdit     = !!id || !!MOCK_COURSE;
  const courseId   = useRef(MOCK_COURSE?.id ?? generateId());
  ...
  const [course, setCourse] = useState<CourseAttributes>({
    ...
    document:     MOCK_COURSE?.document     ?? null,
    image_url:    MOCK_COURSE?.image_url    ?? null,
    isSpecialized: MOCK_COURSE?.isSpecialized ?? false,
    categorie:    MOCK_COURSE?.categorie    ?? "",
  });

  const [thumbFile,   setThumbFile]   = useState<File | null>(null);
  const [docFile,     setDocFile]     = useState<File | null>(null);
  const [thumbDrag,   setThumbDrag]   = useState(false);
  const [categories,  setCategories]  = useState<CategorieAttributes[]>(MOCK_CATEGORIES);
  ...
  const updateLesson = (lessonId: string, field: keyof LessonAttributes, value: string) =>
  ...
  // ── Validation ─────────────────────────────────────────────────────────────
  const courseValid  = course.title.trim() && course.description.trim() && course.categorie;
  const lessonsValid = lessons.every(l => l.title.trim() && l.vedio_url.trim());
  const canPublish   = courseValid && lessonsValid && lessons.length > 0;

  // ── Save ───────────────────────────────────────────────────────────────────
  const handleSave = (status: "draft" | "published") => {
    if (status === "published" && !canPublish) return;
    if (status === "draft"     && !courseValid) return;
    setSaving(true);

    const payload = {
      course:  { ...course, id: courseId.current },
      lessons,
      status,
    };
    console.log("Saving course:", payload);
    // TODO: if isEdit → PUT /api/courses/:id   else → POST /api/courses
    // TODO: PUT/POST /api/lessons for each lesson
  ...
  const lessonIssues = lessons.filter(l => !l.title.trim() || !l.vedio_url.trim()).length;
            ...
            >
              ...
              {tab === "lessons" && (
                <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full
                  ...
                  {lessons.length}
                  {lessonIssues > 0 && ` · ${lessonIssues} incomplete`}
                </span>
              )}
            </button>
          ))}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
            ...
            <div className="lg:col-span-2 flex flex-col gap-5">
              ...
              <SectionCard title="Basic Information" icon={
                ...
                {/* Category + Specialized row */}
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <Label text="Category" required />
                    <select className={inputCls} value={course.categorie}
                      onChange={e => updateCourse("categorie", e.target.value)}>
                  ...
                  <div>
                    ...
                    <div
                      onClick={() => updateCourse("isSpecialized", !course.isSpecialized)}
            ...
            <div className="flex flex-col gap-5">
              ...
              <SectionCard title="Course Thumbnail" icon={
                ...
                >
                  {thumbFile || course.image_url ? (
                    <div className="relative w-full">
                      {thumbFile
                        ? <img src={URL.createObjectURL(thumbFile)} alt="thumbnail" className="w-full h-40 object-cover" />
                        : <div className="w-full h-40 bg-purple-100 flex items-center justify-center text-purple-400 text-sm font-medium">{course.image_url}</div>
                      }
              ...
              <SectionCard title="Preview" icon={
                ...
                <div className="bg-white rounded-xl p-4 border border-gray-100 shadow-sm">
                  ...
                  <div className="flex items-center gap-2 mb-3">
                    <div className="w-7 h-7 rounded-full bg-purple-100 flex items-center justify-center text-purple-700 text-[10px] font-bold shrink-0">DAK</div>
                    <div><p className="text-xs font-semibold text-gray-700">Dr. Ahmed Khalil</p><p className="text-[10px] text-gray-400">Teacher</p></div>
                  </div>
            ...
            {lessonIssues > 0 && (
              <div className="flex items-center gap-3 bg-amber-50 border border-amber-200 rounded-2xl px-4 py-3">
                ...
                <p className="text-xs text-amber-700 font-medium">
                  {lessonIssues} lesson{lessonIssues > 1 ? "s" : ""} missing title or video URL — required to publish.
                </p>
              </div>
            )}
